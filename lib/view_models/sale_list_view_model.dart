...
class SaleListViewModel with ChangeNotifier{
  Future<List<Sale>?> getListSales({required bool isRefresh, required int currentPage, required int payrollCompanyId, int? payrollId, int? saleId, int? limit}) async {
    List<Sale>? result = await ApiService().getListSales(isRefresh: isRefresh, currentPage: currentPage, limit: limit, payrollId: payrollId, saleId: saleId, payrollCompanyId: payrollCompanyId);
