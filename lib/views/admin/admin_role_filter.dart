...
class AdminRoleFilter extends StatefulWidget {
  const AdminRoleFilter({Key? key, this.isAccountDetailFilter, this.isAdminFilter, this.isHrInternFilter, this.isCreateAccount}) : super(key: key);
...
class _AdminRoleFilterState extends State<AdminRoleFilter> {
  ...
  void _getallRoles() async {
    ...
    }else if(widget.isCreateAccount == true){
      for(int i = 0; i < roleList.length; i++){
        if(roleList[i].name == 'Quản trị viên') {
          roleList.removeAt(i);
        }
